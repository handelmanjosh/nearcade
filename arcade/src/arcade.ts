...
import { NearBindgen, near, call, view, LookupMap, UnorderedMap, assert, UnorderedSet, Vector, AccountId, NearPromise, initialize, ONE_YOCTO, ONE_NEAR, ONE_TERA_GAS } from 'near-sdk-js';
...
class Arcade {
  ...
  // added after hackathon - not in webapp
  @call({})
  checkPhysicalPrizeDeliveryStatus({ id }: {id: string}) {
    const prize = this.physicalPrizes.get(id);
    assert(prize, "Prize does not exist");
    assert(prize.player_id === near.predecessorAccountId(), "You do not own this prize");
    assert(prize.tracking_num, "Tracking number has not been set yet")
    // check to see if prize has been delivered
    // if so, then give player the ticket reward
    const url = "https://secure.shippingapis.com/ShippingAPI.dll";
    const xmlRequest = `
      <TrackRequest USERID="592ICARUY0633">
       <TrackID ID="${prize.tracking_num}"></TrackID>
      </TrackRequest>
    `
  ...
  .then(response => {
      ...
      .then((parsedResponse: any) => {
        if (JSON.stringify(parsedResponse).toLowerCase().includes("delivered")) {
          this.ticketBalances.set(near.predecessorAccountId(), (this.ticketBalances.get(near.predecessorAccountId()) || 0) + prize.ticket_reward);
          this.physicalPrizes.remove(id);
        } else {
          assert(false, "Prize has not been delivered yet");
        }
      })
      .catch(parseError => {
          assert(false, "Error parsing xml response");
      });
  })
  .catch(error => {
      console.error('Error:', error);
      assert(false, "Error getting tracking info")
  });
  }
  // end of added after hackathon
  @call({})
  createListing({ id, price, type, contract_id, img_src }: { id: string, price: number, type: string, contract_id: string, img_src: string }) {
      ...
      // not called in testing version, added after hackathon
      if (listing.type === "nft") {
        NearPromise.new(near.currentAccountId()).functionCall(
          "nft_transfer",
          `"reciever_id": ${near.predecessorAccountId()}, "token_id": ${listing.id}`,
          ONE_YOCTO,
          ONE_TERA_GAS
        )
        ...
        NearPromise.new(near.currentAccountId()).functionCall(
          "ft_transfer",
          `"reciever_id": ${near.predecessorAccountId()}, "amount": ${1}`,
          ONE_YOCTO,
          ONE_TERA_GAS,
        )
      } 
      // not called in testing version, end of added after hackathon
  }
  @call({})
  buyListing({ id }) {
    ...
    // not in testing version, added after hackathon
    if (listing.type === "nft") {
      NearPromise.new(near.currentAccountId()).functionCall(
        "nft_transfer",
        `"reciever_id": ${near.predecessorAccountId()}, "token_id": ${listing.id}`,
      ...
      const physicalPrize = new PhysicalPrize({id: generateRandomUUID(), tracking_num: "", player_id: listing.seller, ticket_reward: listing.price});
      this.physicalPrizes.set(id, physicalPrize);
  ...
  buyFromGameShop({game_id, name}: {game_id: string, name: string}) {
    ...
    // these are not called in the testing version of the app
    // added after hackathoin
    if (listing.type === "physical") {
      const physicalPrize = new PhysicalPrize({id: generateRandomUUID(), tracking_num: "", player_id: game.admin, ticket_reward: listing.price});
      this.physicalPrizes.set(physicalPrize.id, physicalPrize);
    } else if (listing.type === "nft") {
      NearPromise.new(near.currentAccountId()).functionCall(
        "nft_transfer",
        `"reciever_id": ${near.predecessorAccountId()}, "token_id": ${listing.name}`,
        ONE_YOCTO,
        ONE_TERA_GAS
      )
    } else if (listing.type === "coin") {
      NearPromise.new(near.currentAccountId()).functionCall(
        "ft_transfer",
        `"reciever_id": ${near.predecessorAccountId()}, "amount": ${listing.name}`,
        ONE_YOCTO,
        ONE_TERA_GAS,
      )
    }
  ...
  addToGameShop({game_id, name, description, price, type, img_src}: {game_id: string, name: string, price: number, description: string, type: string, img_src: string}) {
    ...
    // not called in testing version
    // added after hackathon
    if (type === "nft") {
      NearPromise.new(near.predecessorAccountId()).functionCall(
        "nft_transfer",
        `"reciever_id": ${near.currentAccountId()}, "token_id": ${name}`,
        ONE_YOCTO,
        ONE_TERA_GAS
      )
    } else if (type === "coin") {
      NearPromise.new(near.predecessorAccountId()).functionCall(
        "ft_transfer",
        `"reciever_id": ${near.currentAccountId()}, "amount": ${1}`,
        ONE_YOCTO,
        ONE_TERA_GAS,
      )
    }
  ...
  createGame({
   ...
   NearPromise.new(near.currentAccountId()).transfer(BigInt(1) * ONE_NEAR)
  ...
  fundGame({name, amount}:{name: string, amount: number}) {
    ...
    // added after hackathon
    NearPromise.new(near.currentAccountId()).transfer(BigInt(amount) * ONE_NEAR)
    // end of added after hackathon
  ...
  playGame({ gameId }: { gameId: string }) {
    ...
    assert(game, "Game does not exist")
    // added after hackathon
    NearPromise.new(near.currentAccountId()).transfer(BigInt(game.cost_to_play) * ONE_NEAR)
    // end of added after hackathon
